...
class TestOperations(unittest.TestCase):
    ...
    # Test without plus or minus sign
    @patch("builtins.input",
           return_value="7000")
    def test_bad_input(self, mock_input):
        initial_value = self.operations.registers[4]
        success = self.operations.IO_op(10, 4)
        self.assertFalse(success)   # Assert that the input was not processed
        self.assertEqual(self.operations.registers[4], initial_value)   # Assert that the register value is unchanged
